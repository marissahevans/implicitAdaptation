function h = plot_gaussian_ellipsoid(m, C, color, sdwidth, npts, axh)
...
switch numel(m)
   case 2, h=show2d(m(:),C,color,sdwidth,npts,axh);
   case 3, h=show3d(m(:),C,color,sdwidth,npts,axh);
   otherwise
...
h = patch(bp(1,:), bp(2,:),color, 'FaceAlpha',0.5,'HandleVisibility','off',EdgeColor='none');
%-----------------------------
